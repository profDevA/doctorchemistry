...
class Rokanthemes_Revolutionslideshow_Adminhtml_RevolutionslideshowController extends Mage_Adminhtml_Controller_Action
{
	...
	protected function _initAction() {
		$this->loadLayout()
			->_setActiveMenu('rokanthemes/revolutionslideshow/items')
	...
	public function indexAction() {
		$this->_initAction()
			->_addContent($this->getLayout()->createBlock('revolutionslideshow/adminhtml_revolutionslideshow'))
			->renderLayout();
	}

	public function editAction() {
		$id     = $this->getRequest()->getParam('id');
		$model  = Mage::getModel('revolutionslideshow/revolutionslideshow')->load($id);
		...
		if ($model->getId() || $id == 0) {
			...
			Mage::register('revolutionslideshow_data', $model);

			$this->getLayout()->getBlock('head')->setCanLoadExtJs(true);

			$this->_addContent($this->getLayout()->createBlock('revolutionslideshow/adminhtml_revolutionslideshow_edit'))
				->_addLeft($this->getLayout()->createBlock('revolutionslideshow/adminhtml_revolutionslideshow_edit_tabs'));
	...
	public function saveAction() {
		if ($data = $this->getRequest()->getPost()) {
			...
			$files = array('image','banner_1_img', 'banner_2_img', 'banner_1_imgX2', 'banner_2_imgX2');
			foreach ($files as $_file) {
				if(isset($_FILES[$_file]['name']) and (file_exists($_FILES[$_file]['tmp_name']))) {
					try {
						...
						$path = Mage::getBaseDir('media') .DS.'rokanthemes'.DS.'slideshow'.DS ;
					...
					$data[$_file] = 'rokanthemes/slideshow/'.$result['file'];
				}
				else {
					// handle delete image
			...
			$model = Mage::getModel('revolutionslideshow/revolutionslideshow');
			$model->setData($data)
    ...
    public function massDeleteAction() {
        $revolutionslideshowIds = $this->getRequest()->getParam('revolutionslideshow');
        if(!is_array($revolutionslideshowIds)) {
            ...
            try {
                foreach ($revolutionslideshowIds as $revolutionslideshowId) {
                    $revolutionslideshow = Mage::getModel('revolutionslideshow/revolutionslideshow')->load($revolutionslideshowId);
    ...
    public function massStatusAction()
    {
        ...
        if(!is_array($revolutionslideshowIds)) {
            ...
            try {
                foreach ($revolutionslideshowIds as $revolutionslideshowId) {
                    $revolutionslideshow = Mage::getSingleton('revolutionslideshow/revolutionslideshow')
